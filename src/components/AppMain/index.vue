...
watch(routes, (route) => {
  ...
  const token = Cookies.get(import.meta.env.VITE_TOKEN); //判断已过期
  if (!token) {
    if (!ticket) {
...
const verifyTicket = async (ticket, service) => {
  try {
    ...
    Cookies.set(import.meta.env.VITE_TOKEN, token.value);
    document.cookie = `${import.meta.env.VITE_TOKEN}=` + token.value + ";domain=.gpyh.com;path=/";
